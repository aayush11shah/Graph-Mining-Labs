...
{
	int s = CSR_obj->IA[u] + 1;
	int e = CSR_obj->IA[u + 1];
	while (s <= e)
	{
		int mid = (s + e) / 2;
		if (CSR_obj->JA[mid] == v)
...
{
	return CSR_obj->IA[i] - CSR_obj->IA[i - 1] < CSR_obj->IA[j] - CSR_obj->IA[j - 1] || (CSR_obj->IA[i] - CSR_obj->IA[i - 1] == CSR_obj->IA[j] - CSR_obj->IA[j - 1] && i < j);
}
...
void add_edges(map<int, vector<int>> &adjList, vector<pair<int, int>> &edges)
{
...
void initialize_CSR(CSR *CSR_obj, int n, map<int, vector<int>> &adjList)
{

	for (int i = 0; i < 2 * m; i++)
	{
		CSR_obj->A.push_back(1);
	}

	CSR_obj->IA.push_back(0);
	int prev = 0;
	for (auto x : adjList)
	{
		int u = x.first;
		...
		for (int j = 0; j < v.size(); j++)
			CSR_obj->JA.push_back(v[j]);
		CSR_obj->IA.push_back(prev + v.size());
		prev += v.size();
	}
}
...
void readGraph()
{
	...
	if (myfile.is_open())
	{
		...
		for (auto x : edges)
		{
			...
			if (u == v)
				continue;
			if (csr_rep[u].find(v) == csr_rep[u].end())
			{
				csr_rep[u][v] = 0;
				csr_rep[v][u] = 0;
			}
...
void countTriangles()
{
	...
	{
		int v = mapto[vi];
		for (EdgeIter it = csr_rep[v].begin(); it != csr_rep[v].end(); ++it)
		{
			int u = it->first;
			if (!compVertex(u, v))
				continue;
	...
	fout << total_triangles << " triangles found.\n";
}
...
void binSort()
{
	...
	binEdge.resize(m);
	for (int u = 0; u < n; ++u)
		for (EdgeIter it = csr_rep[u].begin(); it != csr_rep[u].end(); ++it)
		{
			int v = it->first;
			if (!compVertex(u, v))
				continue;
...
void trussDecomp()
{
	for (int s = 0; s < m; ++s)
	{
		...
		for (EdgeIter it = csr_rep[u].begin(); it != csr_rep[u].end(); ++it)
		{
			...
			if (w == v)
				continue;
			if (csr_rep[v].find(w) != csr_rep[v].end())
			{
				++nfound;
...
int main()
{
	...
	readGraph();
	add_edges(adjList, edges);
	initialize_CSR(CSR_obj, n, adjList);
	reorder();
	countTriangles();
	binSort();
	trussDecomp();
